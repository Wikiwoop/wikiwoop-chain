...
build:
	...
	go build -o wikiwoop-chain -ldflags="\
    	-X 'github.com/wikiwoop/wikiwoop-chain/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/wikiwoop/wikiwoop-chain/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/wikiwoop/wikiwoop-chain/versioning.Branch=$(BRANCH)'\
		-X 'github.com/wikiwoop/wikiwoop-chain/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/wikiwoop-chain 
	env CHAIN_BINARY=${PWD}/artifacts/wikiwoop-chain go test -v -timeout=30m ./e2e/...
