...
// call WRC20 contract method and encode parameters
func NewWRC20Generator(params *GeneratorParams) (*ContractTxnsGenerator, error) {
	...
	if gen.encodedParams, err = params.ContractArtifact.ABI.Methods["transfer"].Encode(
		...
		return nil, fmt.Errorf("cannot encode WRC20 transfer method params: %w", err)
	}

	return gen, nil
}

// call WRC721 contract method and encode parameters
func NewWRC721Generator(params *GeneratorParams) (*ContractTxnsGenerator, error) {
	gen := &ContractTxnsGenerator{}
	...
	if gen.encodedParams, err = params.ContractArtifact.ABI.Methods["createNFT"].Encode(
		...
		return nil, fmt.Errorf("cannot encode WRC721 createNFT method params: %w", err)
	}
